...
# Combined function to generate both plots and scorecards in a single figure
plot_fungicide_analysis <- function(results, 
                                    y_multiplier_linear = 1.1, 
                                    y_multiplier_log = 1.05,
                                    line_spacing_factor = 1.0,
                                    asterisk_size = 1.2) {
  
  # Set up a 2x2 layout matrix with specific width and height ratios
  # Main plots will be larger than the scorecards
  layout_matrix <- matrix(c(1, 2, 
                            3, 4), ncol=2, byrow=TRUE)
  
  # Define relative widths and heights for the layout
  # Reduce the height ratio to bring scorecards closer to plots
  layout(layout_matrix, 
         widths = c(1, 1),        # Equal width for columns
         heights = c(2, 0.8))     # Reduced ratio to bring scorecards closer
  
  #------------------------------------------------------------------
  # PLOT 1: ALL TREATMENTS ONLY (ADJUSTED DATA) - Top Left
  #------------------------------------------------------------------
  ...
  # Reduce bottom margin to bring plots closer to scorecards
  par(mar = c(4, 5, 2, 2))  # Reduced bottom margin from 5 to 4
  ...
  plot(NULL, xlim = range(days), 
       ylim = c(max(y_min, 0.001), y_max * upper_limit_multiplier),
       xlab = "Day", ylab = paste0("Adjusted ", y_label), 
       main = "", # Remove title to reduce space
       xaxt = "n", # we'll add custom x-axis
       log = log_param,
       cex.lab = 1.2, cex.axis = 1.1)
  ...
  #------------------------------------------------------------------
  # PLOT 2: ADJUSTED DATA BY CONCENTRATION - Top Right
  #------------------------------------------------------------------
  ...
  #------------------------------------------------------------------
  # SCORECARD 1: TREATMENT SCORECARD - Bottom Left
  #------------------------------------------------------------------
  ...
  # Reduce top margin to bring scorecard closer to plot
  par(mar = c(2, 4, 1, 1))  # Reduced top margin from 3 to 1
  ...
  #------------------------------------------------------------------
  # SCORECARD 2: CONCENTRATION SCORECARD - Bottom Right
  #------------------------------------------------------------------
  ...
  for (i in 1:length(concentrations)) {
    # Format concentration with fewer digits
    conc_label <- format(concentrations[i], scientific = FALSE, digits = 2)
    text(i, 0.3, conc_label, cex = 0.8, srt = 0, adj = c(1, 1))
  }
  ...
  # Return color schemes for potential future use
